...
public class Server {
    ...
    public static void main(String args[]) throws Exception {
        ...
        while (true) {
            ...
            if (totalMessages == 0) {
                if (ls.debugOn) System.out.println(new Date() + "> " + folder + " is empty");
                folder.close(false);
                store.close();
                done = true;
            }
            ...
            if (!done) {
                ...
                for (int i = 0; i < messages.length; i++) {
                    if (!messages[i].isSet(Flags.Flag.SEEN)) {
                        ...
                        else newMessage.setFrom(new InternetAddress(from));
